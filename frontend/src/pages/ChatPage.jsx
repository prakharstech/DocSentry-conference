...
import { Send, Shield, Search, Loader2, AlertCircle, CheckCircle2, FileText, Info, MessageSquare, Trash2, ChevronRight } from 'lucide-react';
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setMessages([...messages, { 
        sender: 'ai', 
        text: `### ✅ Document Processed\n\n**ScannerAgent** identified **${data.pii_count}** PII entities across **${data.pii_types.length}** categories. The document has been anonymized and stored in the FAISS vector store.\n\n**Detected Types:** ${data.pii_types.join(', ')}`,
        type: 'success'
      }]);
    } catch (error) {
                    ...
                    {isReady ? (
                        <div className="space-y-4">
                            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-sm font-black text-green-600">--</p>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-[8px] font-bold text-slate-400 uppercase">Risk</p>
                                        <p className="text-sm font-black text-amber-600">--</p>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-[8px] font-bold text-slate-400 uppercase">Utility</p>
                                        <p className="text-sm font-black text-indigo-600">--</p>
                                    </div>
                                </div>
                            </div>
                            
                            <div className="bg-blue-600 p-4 rounded-2xl text-white shadow-lg shadow-blue-100">
                                <h4 className="text-[10px] font-black uppercase mb-2 opacity-80">Scientific Benchmark</h4>
                                <p className="text-[10px] leading-relaxed mb-4 opacity-90">Run the full Phase 3 evaluation suite on this document.</p>
                                <button 
                                    onClick={() => window.location.href = '/evaluation'}
                                    className="w-full py-2 bg-white text-blue-600 rounded-xl text-[10px] font-black uppercase tracking-wider hover:bg-blue-50 transition-all flex items-center justify-center gap-2"
                                >
                            ...
                            <p className="text-[8px] text-slate-400 leading-normal italic">
                                *All 9 parameters are calculated in the full dashboard using original vs anonymized data.
                            </p>
                        </div>
                    ) : (
